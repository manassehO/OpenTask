...
pub trait IOpenTask<TContractState> {
    ...
    /// - Computes `total_funded_amount` internally as `reward_per_completion * required_completions` when needed.
    /// - Authorization: caller must be the `creator` (enforced in implementation).
    /// - Emits: TaskCreated(task_id, creator, token_address, reward_per_completion, required_completions)
    /// - State: initializes storage entry for the task in `Active` or `DRAFT` equivalent depending on design.
    fn create_task(
    ...
    ///
    /// create and fund a task in a single flow.
    /// - Transfers/escrows total amount equivalent to `reward_per_completion * required_completions` (plus fees off-chain if applicable).
    ...
    /// - Implementation should track claims on-chain if enforced on-chain; otherwise emit reference event.
    ...
    ///
    /// Mirrors backend `task.submitTask`, where `submission_data` can be an off-chain reference (CID/URL/hash).
    ...
    /// - On success, implementation should transfer `reward_per_completion` from escrow to completer and emit RewardPaid.
    /// - `status = true` means approve; if `false`, treat as rejection without payout (or use reject path off-chain).
    /// - Emits: SubmissionApproved(task_id, submission_id), RewardPaid(task_id, completer, submission_id, amount, token)
