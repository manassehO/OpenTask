...
export const earningsRouter = createTRPCRouter({
  ...
  getTransactionHistory: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const userWallet = await ctx.db.query.wallets.findFirst({
        where: (w, { eq, and }) =>
          and(eq(w.userId, user.id), eq(w.isActive, 1)),
      });
      ...
      const earnings =
        input.type !== 'WITHDRAWAL'
          ? await ctx.db.query.onchainEvents.findMany({
              where: (e, { eq }) =>
                eq(e.walletAddress, userWallet.starknetAddress),
              limit: input.limit,
              offset: input.offset,
            })
          : [];
  ...
  createCryptoWithdrawal: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const userWallet = await ctx.db.query.wallets.findFirst({
        where: (w, { eq, and }) =>
          and(eq(w.userId, ctx.user.id), eq(w.isActive, 1)),
      });
      ...
      try {
        ...
        throw new TRPCError({
          ...
          message: 'Failed to create crypto withdrawal',
        });
      }
    }),
