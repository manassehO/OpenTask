...
import { tutorials, user } from '~/server/db/schema';
  ...
  for (const creator of creators) {
    for (let i = 0; i < 3; i++) {
      ...
      try {
        ...
        console.log(
          `Tutorial created for ${creator.email}: ${title} [${category}]`,
        );
      } catch (err: any) {
        console.error(
          `Error creating tutorial for ${creator.email}:`,
          err.message,
        );
      }
    }
