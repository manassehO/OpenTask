  ...
  for (const claim of claims) {
    try {
      ...
      if (Math.random() < 0.8) {
        ...
        const status =
          submissionStatuses[
            Math.floor(Math.random() * submissionStatuses.length)
          ];
        const now = new Date();
        ...
        await db.insert(submissions).values({
          ...
          rejectionReason:
            status === 'REJECTED' ? 'Does not meet requirements' : null,
          approvalTxHash:
            status === 'APPROVED'
        ...
        console.log(
          `Submission created for task ${claim.taskId} by user ${claim.userId} [${status}]`,
        );
      }
    } catch (err: any) {
      console.error(
        `Error creating submission for task ${claim.taskId} by user ${claim.userId}:`,
        err.message,
      );
    }
  }
