...
import { user, userWithdrawalMethods } from "~/server/db/schema";
    ...
    for (let i = 0; i < 2; i++) {
      ...
      const details =
        method === 'CRYPTO_WALLET'
          ? JSON.stringify({ address: `0xWallet${i}${userItem.id.slice(0,6)}` })
          : JSON.stringify({ accountNumber: `12345${i}`, bankName: "Sample Bank" });
      ...
      try {
        ...
        console.log(`Withdrawal method created for user ${userItem.id}: ${name} [${method}]`);
      } catch (err) {
        console.error(`Error creating withdrawal method for user ${userItem.id}:`, err);
      }
    }
  }
