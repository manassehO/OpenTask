...
import { disputes, user } from '~/server/db/schema';
...
async function seedDisputes() {
  ...
  for (const submission of allSubmissions) {
    ...
    if (Math.random() < 0.3) {
      const status =
        disputeStatuses[Math.floor(Math.random() * disputeStatuses.length)];
      const resolution = ['RESOLVED_APPROVE', 'RESOLVED_REJECT'].includes(
        status!,
      )
        ? disputeResolutions[
            Math.floor(Math.random() * disputeResolutions.length)
          ]
        : null;

      const resolvedBy = ['RESOLVED_APPROVE', 'RESOLVED_REJECT'].includes(
        status!,
      )
      ...
      try {
        await db.insert(disputes).values({
          ...
          creatorResponse:
            status !== 'OPEN' ? 'Creator response goes here' : null,
        ...
        console.error(
          `Error creating dispute for submission ${submission.submissionId}:`,
          err.message,
        );
      }
    }
