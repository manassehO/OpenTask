...
import { user, courses } from "~/server/db/schema";
...
async function seedCourses() {
  ...
  for (const creator of creators) {
    for (let i = 0; i < 2; i++) { // 2 courses per creator
      try {
        ...
        console.log(`Course created for ${creator.email}: ${title} [${difficulty}]`);
      } catch (err: any) {
        console.error(`Error creating course for ${creator.email}:`, err.message);
      }
    }
  }
