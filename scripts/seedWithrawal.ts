...
import { user, withdrawals } from "~/server/db/schema";
import { eq } from "drizzle-orm";
import { randomUUID } from "crypto";

const withdrawalMethods = ['CRYPTO_WALLET', 'BANK_ACCOUNT'];
const withdrawalStatuses = ['PENDING', 'PROCESSING', 'COMPLETED', 'FAILED', 'CANCELLED'];
...
async function seedWithdrawals() {
  ...
  for (const userItem of allUsers) {
    ...
    for (let i = 0; i < 2; i++) { // create 2 withdrawals per user
      ...
      const destinationAddress = method === 'CRYPTO_WALLET' ? `0xWallet${i}${userItem.id.slice(0,6)}` : null;
      const bankAccountDetails = method === 'BANK_ACCOUNT' ? JSON.stringify({ accountNumber: `12345${i}`, bank: "Sample Bank" }) : null;
      ...
      try {
        await db.insert(withdrawals).values({
          ...
          bankAccountDetails,
          status,
          txHash: status === 'COMPLETED' ? `0xTxHash${i}${userItem.id.slice(0,6)}` : null,
        ...
        console.log(`Withdrawal created for user ${userItem.id}: ${amount} via ${method} [${status}]`);
      } catch (err) {
        console.error(`Error creating withdrawal for user ${userItem.id}:`, err);
      }
    }
  }
